...
| **data/** | `project_inputs/` (symlink → `inputs/raw_data/`), `past_step_input/` (symlink → the previous step's output), `next_step_output/` (what the next step consumes), and `share/` (a curated dataset you package to hand to a collaborator). |
