    ...
    "tool_audit": {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "effect_size": {"type": "number", "description": "scope='step' dimension='power' — assumed standardized effect size (Cohen's d for t-tests; h for two_proportion; f for anova). No safe default — supply the effect you are powered to detect."},
                "alpha": {"type": "number", "description": "scope='step' dimension='power' — significance level (default 0.05)."},
                "n": {"type": "number", "description": "scope='step' dimension='power' — for two_sample_t / two_proportion this is the PER-GROUP size (nobs1), NOT total N (for a balanced study of total N, pass N/2); for paired_t / one_sample_t / anova it is the per-group / total-per-cell n."},
                "test": {"type": "string", "enum": ["two_sample_t", "paired_t", "one_sample_t", "two_proportion", "anova"], "description": "scope='step' dimension='power' — the analysis design the power is for (default two_sample_t). The reported power is WRONG if this doesn't match the test actually run."},
                "k_groups": {"type": "number", "description": "scope='step' dimension='power' — number of groups; REQUIRED when test='anova'."},
