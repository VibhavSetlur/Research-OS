    ...
    "tool_audit": {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "filepath": {"type": "string", "description": "scope='step' dimensions ∈ {figure, power, assumptions} — file to audit. For power: your power / sample-size JUSTIFICATION (test family, effect size + its source, alpha, n, target power, conclusion). For assumptions: your DIAGNOSTICS RECORD (which assumption checks you ran, the statistics, your interpretation, your response to any violation). RO verifies the record is complete; it does not compute power or run any diagnostic — you run those in your own code."},
                ...
                "effect_size": {"type": "number", "description": "DEPRECATED (ignored): power is no longer computed by RO. Record the effect size and its source inside the filepath justification instead. Accepted for back-compat."},
                "alpha": {"type": "number", "description": "DEPRECATED (ignored): record alpha inside the filepath justification instead. Accepted for back-compat."},
                "n": {"type": "number", "description": "DEPRECATED (ignored): record the sample size inside the filepath justification instead. Accepted for back-compat."},
                "test": {"type": "string", "enum": ["two_sample_t", "paired_t", "one_sample_t", "two_proportion", "anova"], "description": "DEPRECATED (ignored): record the test family inside the filepath justification instead. Accepted for back-compat."},
                "k_groups": {"type": "number", "description": "DEPRECATED (ignored): record the number of groups inside the filepath justification instead. Accepted for back-compat."},
                "risk_ratio": {"type": "number", "description": "scope='step' dimension='evalue' — observed effect on the RISK-RATIO scale. Convert a common-outcome odds ratio first (RR≈sqrt(OR)); for a rare outcome (<~15%) OR≈RR≈HR so the raw value may be used."},
                "ci_lower": {"type": "number"},
                "ci_upper": {"type": "number"},
                "effect_measure": {"type": "string", "enum": ["rr", "or", "hr"], "description": "scope='step' dimension='evalue' — scale of risk_ratio (default 'rr'). 'or' on a common outcome is converted RR≈sqrt(OR); 'hr' is flagged (convert manually unless rare_outcome)."},
                "rare_outcome": {"type": "boolean", "description": "scope='step' dimension='evalue' — set true for a rare outcome (<~15%), where OR≈HR≈RR and the value is used directly."},
