...
def discussion_coverage_audit(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for r in non_agrees:
            # v1.5.0 stress audit fix: word-boundary regex match (was
            # substring `in disc_text`, which over-credited stem-prefix
            # words like "expr" hitting "expression"). Also: when a claim
            # has fewer than 2 keywords, require ALL of them — previous
            # `max(2, ...)` made short claims like "BMI rises" or
            # "Cox PH fits" unprovably uncovered.
            claim_words = [
