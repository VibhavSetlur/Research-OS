...
def _project_python_requirements(root: Path, step_id: str | None = None) -> str:
    ...
    header = [
        ...
        "# versions installed when the env snapshot last ran. Regenerate",
...
def _render_language_recommendations(
    ...
    lines = [
        ...
        "",
        "*Auto-generated by `sys_env(operation='snapshot')` from the files in "
    ...
    lines.extend([
        ...
        "   `Project.toml` (Julia).",
        "2. Re-run `sys_env(operation='snapshot')` after `pip install -r requirements.txt` ",
...
def _render_multi_lang_dockerfile_stub(langs: list[str]) -> str:
    ...
    lines = [
        ...
        "# Auto-generated by sys_env(operation='docker_generate') when >= 2 languages were ",
...
def env_docker_generate(root: Path, *, step_id: str | None = None) -> dict[str, Any]:
    """Generate a Dockerfile from an environment snapshot.

    Target rules (mirror ``env_snapshot``):
      * ``step_id`` given → generate ``workspace/<step_id>/environment/Dockerfile``
        + a sibling ``.dockerignore``, built from THAT step's snapshot. The
        image build context is the step directory, so the Dockerfile COPYs the
        step's scripts + data and pip-installs the step's own requirements —
        a self-contained, reproducible container for ONE analysis step.
      * else → the project-global ``environment/Dockerfile`` (whole project).

    Run ``env_snapshot`` first so requirements.txt / session.yaml exist; this
    function reads them but does not regenerate them.
    """
    try:
        if step_id:
            step_root = root / "workspace" / step_id
            if not step_root.is_dir():
                return {
                    "status": "error",
                    "message": f"step_id '{step_id}' not found under workspace/",
                }
            env_dir = step_root / "environment"
            # Paths inside the Dockerfile are relative to the build context,
            # which for a step image is the STEP directory.
            req_rel = "environment/requirements.txt"
            renv_rel = "environment/renv.lock"
            scope_label = f"analysis step {step_id}"
        else:
            env_dir = root / "environment"
            req_rel = "environment/requirements.txt"
            renv_rel = "environment/renv.lock"
        ...
        lines = [
            f"# Reproducible container for the {scope_label}.",
            "# Generated by Research-OS sys_env(operation='docker_generate'"
            + (f", step_id='{step_id}'" if step_id else "") + ").",
            "# Build from this directory:  docker build -t ro-"
            + (step_id or "project").replace("_", "-") + " .",
        ...
        if (env_dir / "requirements.txt").exists():
            lines.append(
                f"RUN pip install --no-cache-dir -r {req_rel}"
            )

        if (env_dir / "renv.lock").exists():
            lines.append(
                "RUN R -e 'install.packages(\"renv\", repos=\"https://cloud.r-project.org\"); "
                f"renv::restore(lockfile=\"{renv_rel}\")'"
            )
        ...
        # A .dockerignore keeps the image lean + reproducible: never bake the
        # machine-local state, caches, or scratch into the image.
        ignore_path = env_dir.parent / ".dockerignore"
        try:
            if not ignore_path.exists():
                ignore_path.write_text(
                    "# Keep the image lean + reproducible.\n"
                    ".os_state/\n__pycache__/\n*.pyc\nscratch/\nlogs/\ncache/\n.git/\n"
                )
        except OSError:
            pass
        ...
        result = {
            ...
            "scope": "step" if step_id else "project",
            "build_context": str((env_dir.parent).relative_to(root)),
            "message": (
                f"Dockerfile generated for {scope_label}. Build it from "
                f"`{(env_dir.parent).relative_to(root)}` "
                f"(docker build -f environment/Dockerfile .) so the step runs "
                "in its own pinned container."
                if step_id else "Dockerfile generated."
            ),
        }
        return result
    except Exception as e:
